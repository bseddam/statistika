...
function getData(PageValue) { 
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var nodata = '<tr><td colspan="3"><center>No data to display</center></td></tr>';
            var html = '';
            if (data == 'error') {
            ...
            for (var i = 0; i < data.length; i++) {
                html += '<tr>' +
                    ...
                    '<td>' + data[i].Mail + '</td>' +
                    '<td>' + formatJSONDate(data[i].Add_dt) + '</td>' +
                   '</tr>';
            }
...
$(document).on('click', 'a[data-action="delete"]', function () {
    $('.subscribers-delete .alert').css('display', 'none');
    $('.subscribers-delete').attr('data-id', $(this).attr('data-id'));
});
$(document).on('click', 'a[data-action="changeactivity"]', function () {
    $('.subscribers-changeactivity .alert').css('display', 'none');
    $('.subscribers-changeactivity').attr('data-id', $(this).attr('data-id'));
    $('.subscribers-changeactivity').attr('data-activity', $(this).attr('data-activity'));
});
$(document).on('submit', '.subscribers-delete', function () {
    Metronic.blockUI({
...
$(document).on('submit', '.subscribers-changeactivity', function () {
    Metronic.blockUI({
        boxed: true, target: '.portlet-body',
        message: jsLangText['loading']
    });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.key == "ok") {
                $('#subscribers-changeactivity').modal('hide').delay(5000);
                getData(currentPage);
