...
function createTable(data, rowCount, langID, categoryID, activity) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (jsPermission['isEdit'] == 'True') {
            html += '<td><a class="btn green" href="/admin/music/edit/' + data[i].ID + '"><i class="fa fa-edit "></i> Edit</a></td> ';
        }
        ...
        if (jsPermission['isActivity'] == 'True') {
            html += '<td><a class="btn default" data-toggle="modal" data-action="changeactivity" data-activity="' + activity + '" data-id="' + data[i].ID + '" href="#music-changeactivity"><i class="fa fa-history"></i> Active</a></td>';
        }
        html += '</tr>';
    }
