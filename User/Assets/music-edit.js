...
$(function () {
    window.setTimeout(function () {
        ...
        editor = $('.music-info-1').find('.note-editor').html();
        for (var i = 2; i <= infoCount; i++) {
            $('.music-info-' + i).find('[name="summernote"]').after('<div class="note-editor">' + editor + '</div>');

            desc = $('.music-info-' + i).find('[name="summernote"]').html();
            $('.music-info-' + i).find('.note-editable').html(desc);

            $('.music-info-' + i).find('[name="summernote"]').html('');
        }
    }, 500);
