...
function getData(PageValue) {
    ...
    categoryedit = $('input[name="categoryedit"]').val();
    Metronic.blockUI({
        boxed: true,
        target: '.portlet-body',
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var nodata = '<tr><td colspan="5"><center>No data to display</center></td></tr>';
            var html = '';
            if (data == 'error') {
            ...
            for (var i = 0; i < data.length; i++) {
                html += '<tr>' +
                    ...
                    '<td>' + data[i].Name + '</td>' +
                    '<td><div style=background-image:url("/uploads/mood/' + data[i].ID + '.jpg");background-size:contain;background-repeat:no-repeat;width:50px;height:50px;margin:auto;></div></td>';
                if (jsPermission['isEdit'] == 'True') {
                    html += '<td><a class="btn green" href="' + categoryedit + '/' + data[i].ID + '"><i class="fa fa-edit "></i> Edit</a></td> ';
                }
                html += '</tr>';
            }
...
$(document).on('click', 'a[data-action="delete"]', function () {
    $('.music-delete .alert').css('display', 'none');
    $('.music-delete').attr('data-id', $(this).attr('data-id'));
});
$(document).on('click', 'a[data-action="changeactivity"]', function () {
    $('.music-changeactivity .alert').css('display', 'none');
    $('.music-changeactivity').attr('data-id', $(this).attr('data-id'));
    $('.music-changeactivity').attr('data-activity', $(this).attr('data-activity'));
});
$(document).on('submit', '.music-delete', function () {
    Metronic.blockUI({
...
$(document).on('submit', '.music-changeactivity', function () {
    Metronic.blockUI({
        boxed: true, target: '.portlet-body',
        message: jsLangText['loading']
    });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.key == "ok") {
                $('#music-changeactivity').modal('hide').delay(5000);
                getData(currentPage);
