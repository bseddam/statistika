...
public partial class WebPages_Publication : System.Web.UI.Page
{
    DALC _db = new DALC();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblPageTitle.Text = DALC.GetStaticValue("publications_page_title");
        Page.Title = lblPageTitle.Text + " " + Config.GetAppSetting("ProjectName");

        ltrBreadCrumb.Text = string.Format("<a href='/{0}/home'> {1}</a> / <a href='/{0}/publication-research'> {3}</a> / {2} ",
                                lang,
        ...
        if (getContentId != 0)
        {
            DataTable dtContent = _db.GetPublicationsById(getContentId);
            if (dtContent != null)
            {
                ...
                DataTable dtCategory = _db.GetPublicationCategroyById(dtContent.Rows[0]["category_id"].ToParseInt());


                ltrBreadCrumb.Text = string.Format("<a href='/{0}/home'> {1}</a> / <a href='/{0}/publication-research'> {4}</a> / <a href='/{0}/publication'>{2}</a> / <a href='/{0}/publication/list/{6}'>{5}</a> / {3}",
                     lang,
                     DALC.GetStaticValue("home_breadcrumb_title"),
                     Config.HtmlRemoval.StripTagsRegex(lblPageTitle.Text),
                     Config.HtmlRemoval.StripTagsRegex(lblContentTitle.Text),
                     DALC.GetStaticValue("home_box_4"),
                     dtCategory.Rows[0]["name_" + lang].ToParseStr(),
                     dtContent.Rows[0]["category_id"].ToParseStr()
                );
    ...
    {
        rptPosts.DataSource = _db.GetPublicationsAll(page, rowCount);
        rptPosts.DataBind();
    }
    void loadPager()
    {
        ...
        int totalCount = _db.GetPublicationsAllCount();
