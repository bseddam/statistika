...
public partial class WebPages_Page : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        DataTable dt = _db.GetPagesByID(pageId);
        if (dt.Rows.Count < 1)
        {
            Config.Rd("/error");
        }

        ltrTitle.Text = dt.Rows[0]["title_" + lang].ToParseStr();
        ltrContent.Text = dt.Rows[0]["content_" + lang].ToParseStr();

        Page.Title = Config.HtmlRemoval.StripTagsRegex(dt.Rows[0]["title_" + lang].ToParseStr()) + " - " + Config.GetAppSetting("ProjectName");
