...
public partial class Admin_Slider_Add : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (_dataID > 0)
        {
            ...
            LtrPageTitle.Text = "Faydalı link başlığı məlumatlarını yenilə";
        }
    ...
    private void _loadDataByID(int DataID)
    {
        ...
        DataTable dt = _db.getlinkheader(DataID);

        if (dt == null && dt.Rows.Count < 1) return;


        useful_links_headers_az.Html = dt.Rows[0]["useful_links_header_az"].ToParseStr();
        useful_links_headers_en.Html = dt.Rows[0]["useful_links_header_en"].ToParseStr();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (useful_links_headers_az.Html.Trim().Length < 3)
        {
        ...
        {
            returnVal = _db.UsefullinkheaderUpdate(_getIDFromQuery,
                useful_links_headers_az.Html,
                useful_links_headers_en.Html,
                OrderBy.Value.ToParseInt());
        }
        else
        {
            returnVal = _db.UsefullinkheaderInsert(
              useful_links_headers_az.Html,
              useful_links_headers_en.Html,
              OrderBy.Value.ToParseInt());
            ...
            //content.Id = pageId;
            //content.Name = useful_links_headersurl_az.Text.Trim();
            //content.Content = useful_links_headers_az.Html;
