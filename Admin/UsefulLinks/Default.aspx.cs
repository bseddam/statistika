...
public partial class Admin_Slider_Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ltrPageTitle.Text = "Faydalı linklər";
    ...
    void _loadGridFromDb()
    {
        ...
        GridViewDataComboBoxColumn column = Grid.Columns["useful_links_header_id"] as GridViewDataComboBoxColumn;
        column.PropertiesComboBox.Items.Clear();
        foreach (DataRow item in dtlinkheader.Rows)
        {
            column.PropertiesComboBox.Items.Add(new DevExpress.Web.ASPxEditors.ListEditItem(item["useful_links_header_az"].ToParseStr(), item["id"].ToParseInt()));
        }
        column.PropertiesComboBox.Items.Insert(0, new DevExpress.Web.ASPxEditors.ListEditItem(_defaultDropdownRowText, ""));


        GridViewDataComboBoxColumn column1 = Grid.Columns["useful_links_header_id1"] as GridViewDataComboBoxColumn;
        column1.PropertiesComboBox.Items.Clear();
        foreach (DataRow item in dtlinkheader.Rows)
        {
            column1.PropertiesComboBox.Items.Add(new DevExpress.Web.ASPxEditors.ListEditItem(item["useful_links_header_en"].ToParseStr(), item["id"].ToParseInt()));
        }
        column1.PropertiesComboBox.Items.Insert(0, new DevExpress.Web.ASPxEditors.ListEditItem(_defaultDropdownRowText, ""));


        DataTable dt = _db.GetUsefullLinks();
    ...
    protected void LnkPnlMenu_Click(object sender, EventArgs e)
    {
        ...
        {
            case "add":
                Config.Rd("/admin/usefullinks/add.aspx");
                break;
            //case "search":
